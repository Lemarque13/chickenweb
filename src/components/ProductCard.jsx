...
const ProductCard = ({ product }) => {
  // Достаем из хранилища всё, что нам нужно
  const { items, favorites, addToCart, removeFromCart, toggleFavorite } = useCartStore();

  const itemInCart = items.find((item) => item.$id === product.$id);
  // Проверяем, есть ли ID этого товара в массиве избранных
  ...
  return (
    <div className="product-card">
      <div className="product-image-container">
        {/* При клике на сердечко вызываем toggleFavorite */}
        <div className="favorite-icon" onClick={() => toggleFavorite(product.$id)}>
          {/* Меняем цвет заливки в зависимости от isFavorite */}
